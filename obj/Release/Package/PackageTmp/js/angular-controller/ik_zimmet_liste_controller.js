angular.module('inspinia').controller(
    'ik_zimmet_liste_controller', ['$scope', '$http', '$state', '$stateParams', '$rootScope', '$modal', 'ngDialog', 'srvRaporlar', 'srvIkZimmet', 'srvGenel', 'Constants', 'Ayarlarim',
        function ($scope, $http, $state, $stateParams, $rootScope, $modal, ngDialog, srvRaporlar, srvIkZimmet, srvGenel, Constants, Ayarlarim) {
            ...
            $scope.init = function () {
                $scope.IkZimmetGetData();
            };

            $scope.IkZimmetGetData = function () {
                ...
                promiseGet.then(function (gelen) {
                    ...
                    if (gelen.data.Veri.length > 0 && gelen.data.Veri[0].basariDurumu === false) {
                        mesajGoster('Dikkat', 'Zimmet listesi yüklerken bir hata oluştu.' + gelen.data.Veri[0].mesaj, gelen.data.Veri[0].sistemMesaj !== null ? 'E' : 'W');
                        console.error('Zimmet listesi yüklenirken bir hata oluştu. Hata:', gelen.data.Veri[0].sistemMesaj);
                    } else {
                        $scope.toplamKayitSayisi = gelen.data.ToplamKayitSayisi;
                        $scope.IkZimmetListesi = gelen.data.Veri;
                    }
                },
                    function (hata) {
                        $rootScope.sayfayukleniyor = false;
                        ...
                        console.error('Zimmet listesi yüklenirken bir hata oluştu. Hata:', hata);
                    });
            };
            ...
            $scope.IkZimmetSil = function (info) {
                $rootScope.sayfayukleniyor = true;
                var promiseGet = srvIkZimmet.IkZimmetSil(info.IK_ZIMMET_ID);
                promiseGet.then(function (gelen) {
                    $rootScope.sayfayukleniyor = false;
                    if (gelen.data.basariDurumu === false) {
                        mesajGoster('Dikkat', 'Zimmet silme işlemi sırasında bir hata oluştu.' + gelen.data.mesaj, gelen.data.sistemMesaj !== null ? 'E' : 'W');
                        console.error('Zimmet silme işlemi sırasında bir hata oluştu. Hata:', gelen.data.sistemMesaj);
                    }
                    else {
                        mesajGoster('İşlem tamam.', "Zimmet silme işleminiz başarılı bir şekilde gerçekleştirilmiştir.", 'S');
                        if ($scope.IkZimmetListesi.length == 1 && $scope.toplamKayitSayisi > 10) {
                    ...
                    function (hata) {
                        ...
                        console.error('Zimmet silme işlemi sırasında bir hata oluştu. Hata:', hata);
                    });
            };
            ...
            $scope.modalSilmeOnayi = function (info) {
                ...
                ngDialog.openConfirm({
                    ...
                    function (value) {
                        if ('true' === value) {
                            $scope.IkZimmetSil($scope.secilenKayit);
            ...
            $scope.filtreTemizle = function () {
                $scope.AramaKriter = {
                    ...
                    IK_ZIMMET_TESLIM_ALAN_KISI_ADI: null,
                    IK_ZIMMET_TESLIM_EDEN_KISI_ADI: null,
                    LISTE: true
                };
                $scope.IkZimmetGetData($scope.AramaKriter);
            };
            ...
            $scope.ZimmetFormuYazdir = function (id, formTipi) {
                $scope.Kriter = {
                    ...
                    IK_ZIMMET_TESLIM_ALAN_KISI_ADI: $scope.AramaKriter.IK_ZIMMET_TESLIM_ALAN_KISI_ADI,
                    IK_ZIMMET_TESLIM_EDEN_KISI_ADI: $scope.AramaKriter.IK_ZIMMET_TESLIM_EDEN_KISI_ADI
