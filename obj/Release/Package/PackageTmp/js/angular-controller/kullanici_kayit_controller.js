angular.module('inspinia').controller(
    'kullanici_kayit_controller', ['$scope', '$http', '$state', '$stateParams', '$rootScope', '$modal', '$log', 'srvGenel', 'ngDialog', 'srvYetkilendirme', 'srvKullanici', 'srvMusteri', 'srvKullaniciProje', 'srvProjeler', 'Ayarlarim',
        function ($scope, $http, $state, $stateParams, $rootScope, $modal, $log, srvGenel, ngDialog, srvYetkilendirme, srvKullanici, srvMusteri, srvKullaniciProje, srvProjeler, Ayarlarim) {
            ...
            $scope.KullaniciProjeGetData = function () {
                ...
                promiseGet.then(function (gelen) {
                    ...
                    if (gelen.data.Veri.length > 0 && gelen.data.Veri[0].basariDurumu === false) {
                        ...
                        $scope.toplamKayitSayisiKullaniciProje = gelen.data.ToplamKayitSayisi;
            ...
            $scope.modalKullaniciProjeSilmeOnayi = function (info) {
                ...
                ngDialog.openConfirm({
                    ...
                    function (value) {
                        if ('true' === value) {
                            $scope.KullaniciProjeSil($scope.secilenKayit);
            ...
            $scope.KullaniciProjeSil = function (info) {
                $rootScope.sayfayukleniyor = true;
                var promiseGet = srvKullaniciProje.KullaniciProjeSil(info);
                promiseGet.then(function (gelen) {
                    $rootScope.sayfayukleniyor = false;
                    if (gelen.data.basariDurumu === false) {
                        mesajGoster('Dikkat', 'Kullanıcı proje silme işlemi sırasında bir hata oluştu.' + gelen.data.mesaj, gelen.data.sistemMesaj !== null ? 'E' : 'W');
                        console.error("Kullanıcı proje silme işlemi sırasında bir hata oluştu. Hata:", gelen.data.sistemMesaj);
                    }
                    else {
                        mesajGoster("İşlem tamam.", "Kullanıcı proje silme işlemi başarılı bir şekilde gerçekleştirilmiştir.", "S");
                        if ($scope.KullaniciProjeListesi.length == 1 && $scope.toplamKayitSayisiKullaniciProje > 10) {
                    ...
                    function (hata) {
                        $rootScope.sayfayukleniyor = false;
                        mesajGoster('Dikkat', hata.status === 404 ? hata.data : hata.data.mesaj, 'I');
                        console.error('Kullanıcı proje silme işlemi sırasında bir hata oluştu. Hata:', hata);
                    });
            };

            $scope.KullaniciProjeEkleGuncelle = function (KullaniciProje) {
                $rootScope.sayfayukleniyor = true;
                $scope.formCalistirildiKullaniciProje = true;
                if ($scope.frmKullaniciProje.$valid) { } else {
                    $rootScope.sayfayukleniyor = false;
                    $rootScope.focusToInvalid($scope.frmKullaniciProje);
                    return;
                }
                KullaniciProje.KULLANICI_ID = $scope.kullaniciID;
                var promiseGet = srvKullaniciProje.KullaniciProjeEkleGuncelle(KullaniciProje);
                promiseGet.then(function (gelen) {
                    $rootScope.sayfayukleniyor = false;

                    if (gelen.data.basariDurumu === false) {
                        mesajGoster('Dikkat', 'Kullanıcı proje kayıt işlemi sırasında bir hata oluştu.' + gelen.data.mesaj, gelen.data.sistemMesaj !== null ? 'E' : 'W');
                        console.error('Kullanıcı proje kayıt işlemi sırasında bir hata oluştu. Hata:', gelen.data.sistemMesaj);
                    }
                    else {
                        mesajGoster("İşlem tamam.", "Kullanıcı proje kayıt işleminiz başarılı bir şekilde gerçekleştirilmiştir.", "S");
                        $scope.KullaniciProjeGetData();
                        $scope.formCalistirildiKullaniciProje = false;
                        $scope.KullaniciProje.PROJE_ID = null;
                    }
                },
                    function (hata) {
                        $rootScope.sayfayukleniyor = false;
                        mesajGoster('Dikkat', hata.status === 404 ? hata.data : hata.data.mesaj, 'I');
                        console.error('Kullanıcı proje kayıt işlemi sırasında bir hata oluştu. Hata:', hata);
                    });
            };


            $scope.ProjelerGetData = function () {
                $rootScope.sayfayukleniyor = true;
                $rootScope.sayfayukleniyor = true;
